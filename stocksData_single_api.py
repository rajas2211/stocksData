...
def execute_symbols(symbols, output_file, exchange):
    ...
    for idx, symbol in enumerate(symbols):
        ...
        try:
            ...
            weekly = get_data(symbol, exchange)
            # end_time = time.time()
...
def computeData(symbol, weekly):
    (weekEnd, monthBegin, monthEnd,
     quarterBegin, quarterEnd,
     halfBegin, halfEnd,
     yearBegin, yearEnd) = getDates()
    # print("Dates retrived")
    weeklyData = getDictStandard(weekly, weekEnd, 'weekly')
    monthlyData = getDictCustom(weekly, monthBegin, monthEnd, 'monthly')
    quarterlyData = getDictCustom(weekly, quarterBegin, quarterEnd,
                                  'quarterly')
    halflyData = getDictCustom(weekly, halfBegin, halfEnd, 'halfly')
    yearlyData = getDictCustom(weekly, yearBegin, yearEnd, 'yearly')
...
def getDates():
    ...
    monthBegin = monthEnd + pd.offsets.MonthBegin(n=-1)
    ...
    if not today.is_year_end:
        ...
        halfBegin = pd.Timestamp(datetime.date(halfEnd.year, 7, 1))
    ...
    return (weekEnd, monthBegin, monthEnd,
            quarterBegin, quarterEnd,
            halfBegin, halfEnd,
            yearBegin, yearEnd)
